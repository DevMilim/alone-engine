...
impl NetworkServer {
                    ...
                    Ok((stream, peer_addr)) => {
                        ...
                        tokio::spawn(async move {
                            if let Ok(ws_stream) = accept_async(stream).await {
                                ...
                                let encoded_disconnect =
                                    serialize_bytes(&NetworkEvent::Disconnected);
    ...
    pub fn send(&self, target: SocketAddr, event: ServerEvent) -> Result<(), NetworkError> {
        self.sender.try_send((target, event)).map_err(|e| match e {
